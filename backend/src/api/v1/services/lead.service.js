...
const createLead = async (leadData, detailData) => {
  ...
  try {
    ...
    const predictionData = {
      ...
      housing:
        detailData?.lead_housing_loan === true || detailData?.lead_housing_loan === 1
          ? 'yes'
          : 'no',
...
const bulkInsertWithScore = async (leads, onProgress = null) => {
    ...
    for (let i = 0; i < leads.length; i++) {
      ...
      try {
        ...
        if (successCount % 100 === 0 || successCount === total) {
          console.log(`✅ [DB Insert] Progress: ${successCount}/${total} leads saved...`);
          if (onProgress) {
            onProgress({ saved: successCount, total });
          }
        }
      } catch (err) {
...
const queryLeads = async (queryOptions) => {
  ...
  const segmentId = queryOptions.segmentId;
...
const getAllSegments = async () => {
  const { rows } = await db.query('SELECT * FROM tb_segment ORDER BY segment_id ASC');
  return rows;
};
...
const getSegmentStats = async () => {
  const query = `
    ...
    FROM tb_segment s
    LEFT JOIN tb_leads l ON s.segment_id = l.segment_id
    LEFT JOIN tb_leads_score ls ON l.lead_id = ls.lead_id
    GROUP BY s.segment_id, s.lead_segment
