...
const RankedListCard = ({ title, items, isScore = false }) => (
  <div className="flex flex-col h-full p-5 rounded-lg shadow-lg bg-dark-card">
    ...
    {isScore && (
      <button className="self-start mt-4 text-sm text-orange-500 hover:underline">View All</button>
    )}
  </div>
);
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-4 gap-6 mb-8">
        ...
        />
        <StatCard
          title="Average Leads Score"
          value={dashboardData.avgLeadScore}
          detail="77%"
          detailBg="bg-green-600"
          valueColor="text-white"
        />
        <StatCard
        ...
        <StatCard
          title="Rebound Rate"
          value={dashboardData.reboundRate}
          detail="10%"
          detailBg="bg-red-600"
          valueColor="text-white"
        />
      ...
      {/* Row Bottom */}
      <div className="p-5 mt-10 mb-8 rounded-lg shadow-lg bg-dark-card">
        <h3 className="mb-4 text-lg font-semibold text-white">Conversion Rate Trend</h3>
        <p className="mb-2 text-sm text-gray-400">
          · Define: Grafik garis (line chart) yang menunjukkan pergerakan Conversion Rate dari waktu
          ke waktu (misal: per hari atau per minggu).
        </p>
        <p className="text-sm text-gray-400">
          · Sumber Data: tb_lead_status. Kelompokkan (GROUP BY) berdasarkan changed_at (dijadikan
          harian/mingguan). Hitung rasio status_id = Deal terhadap total status yang berubah pada
          periode tersebut.
        </p>
