  ...
  return createPortal(
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-70">
      <div className="bg-dark-card w-full max-w-xl rounded-2xl shadow-2xl p-8 text-white border border-white/10 relative overflow-visible">
        ...
        >
          <div>
            <label className="block mb-1 text-sm font-medium text-gray-300">Campaign Name*</label>
            <input
              name="campaign_name"
          ...
          <div className="relative">
            <label className="block mb-1 text-sm font-medium text-gray-300">Assign Sales</label>

            <button
              type="button"
              onClick={() => setOpenDropdown((prev) => !prev)}
              className="w-full p-2.5 bg-[#2C2C2C] text-white rounded-lg border border-white/20 flex justify-between items-center"
            >
            ...
            {openDropdown && (
              <div className="absolute z-40 w-full mt-1 bg-[#2C2C2C] border border-white/20 rounded-lg max-h-48 overflow-y-auto shadow-lg">
                {salesList.length === 0 ? (
                  <p className="p-3 text-gray-400 text-sm">No sales available.</p>
                ) : (
                  salesList.map((s) => (
                    <label
                      key={s.user_id}
                      className="flex items-center gap-2 p-2 text-white hover:bg-gray-700 cursor-pointer"
                    >
                      <input
                        type="checkbox"
          ...
          <div className="flex items-center pt-2 space-x-3">
            ...
            <span className="text-sm text-white">Active Campaign</span>
          </div>
