        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar pr-1">
                ...
                <div>
                    <div className="grid grid-cols-2 gap-4">
                        <div className="p-3 bg-gray-50 dark:bg-[#2A2A2A] rounded-lg border border-gray-200 dark:border-white/5 relative overflow-hidden group">
                            ...
                            <p className="font-bold text-lg text-gray-900 dark:text-white tracking-wide">{lead.lead_phone_number || '-'}</p>
                        </div>
                        <div className="p-3 bg-gray-50 dark:bg-[#2A2A2A] rounded-lg border border-gray-200 dark:border-white/5 relative overflow-hidden group">
                             <div className="absolute top-0 left-0 w-1 h-full bg-purple-500/50"></div>
                            <p className="mb-1 text-xs text-gray-500 dark:text-gray-400">Email</p>
                            <p className="font-bold text-base text-gray-900 dark:text-white tracking-wide break-all">{lead.lead_email || '-'}</p>
                        </div>
                    </div>
                </div>
                <div>
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 bg-gray-50 dark:bg-[#2a2a2a] p-4 rounded-xl border border-gray-200 dark:border-white/5">
                        <div>
                            ...
                            <p className="font-medium text-gray-900 dark:text-white capitalize">{lead.job_name || '-'}</p>
                        </div>
                        <div>
                            <p className="mb-1 text-xs text-gray-500 dark:text-gray-400">Education</p>
                            <p className="font-medium text-gray-900 dark:text-white capitalize">{lead.education_level || '-'}</p>
                        </div>
                        <div>
                            <p className="mb-1 text-xs text-gray-500 dark:text-gray-400">Marital Status</p>
                            <p className="font-medium text-gray-900 dark:text-white capitalize">{lead.marital_status || '-'}</p>
                        </div>
                        <div>
                            <p className="mb-1 text-xs text-gray-500 dark:text-gray-400">Age</p>
                            <p className="font-medium text-gray-900 dark:text-white">{lead.lead_age ? `${lead.lead_age} Years` : '-'}</p>
                        </div>
                    </div>
                </div>
                <div>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-center bg-gray-50 dark:bg-[#2a2a2a] p-4 rounded-xl border border-gray-200 dark:border-white/10 shadow-lg">
                        ...
                        <div className="md:col-span-2 flex flex-wrap gap-6 pl-4">
                            <div>
                                <p className="mb-2 text-xs text-gray-500 dark:text-gray-400">Housing Loan</p>
                                <span className={`text-sm font-bold ${lead.lead_housing_loan ? 'text-gray-900 dark:text-white' : 'text-gray-400 dark:text-gray-500'}`}>
                                    {lead.lead_housing_loan ? 'Yes' : 'No'}
                                </span>
                            </div>
                            <div>
                                <p className="mb-2 text-xs text-gray-500 dark:text-gray-400">Personal Loan</p>
                                <span className={`text-sm font-bold ${lead.lead_loan ? 'text-gray-900 dark:text-white' : 'text-gray-400 dark:text-gray-500'}`}>
          ...
          {activeTab === 'log' && (
            <div className="space-y-4 animate-fadeIn px-2">
              <div>
                <label className="text-xs text-gray-700 dark:text-gray-300">Contact Method *</label>
                <select
                  value={method}
              ...
              <div>
                <label className="text-xs text-gray-700 dark:text-gray-300">Contact Date *</label>
                <input
                  type="date"
                  value={date}
              ...
              <div>
                <label className="text-xs text-gray-700 dark:text-gray-300">Call Duration (seconds) *</label>
                <input
                  type="number"
                  placeholder="e.g. 120"
                  value={duration}
              ...
              <div>
                <label className="text-xs text-gray-700 dark:text-gray-300">Outcome / Status *</label>
                <select
                  value={status}
                ...
                >
                  ...
                  <option value="Connected">Connected (Keep Contacted)</option>
                  <option value="No Answer">No Answer (Keep Contacted)</option>
                  <option value="Busy">Busy (Keep Contacted)</option>
                  <option value="Wrong Number">Wrong Number (Keep Contacted)</option>
                  <option value="Accepted">Deal (Finalize)</option>
                  <option value="Rejected">Reject (Finalize)</option>
                </select>
                <p className="text-[10px] text-gray-400 mt-2">
                  <span className="text-brand">Note:</span> Selecting <b>Deal</b> or <b>Reject</b> will finalize this lead.
                </p>
              </div>

              <div className="mt-8 flex justify-end gap-3 pt-4 border-t border-gray-200 dark:border-white/10">
                <button
                    onClick={onClose}
          ...
          {activeTab === 'history' && (
            <div className="animate-fadeIn">
                ...
                ) : history.filter(h => !['Accepted', 'Rejected', 'Deal', 'Reject'].includes(h.outcome)).length > 0 ? (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pb-4">
                    {history
                      .filter(h => !['Accepted', 'Rejected', 'Deal', 'Reject'].includes(h.outcome))
                      .map((item) => (
                    <div
                    ...
                    >
                        ...
                        <div className="flex justify-between items-start mb-3 relative z-10">
                            <div>
312:                                 <span className="text-[10px] text-gray-500 dark:text-gray-400 block mb-1 uppercase tracking-wider font-semibold">
313:                                     {new Date(item.created_at).toLocaleString('id-ID', { day: '2-digit', month: 'short', hour: '2-digit', minute: '2-digit' })}
314:                                 </span>
315:                                 <div className="flex items-center gap-2">
316:                                     <span className="font-bold text-gray-900 dark:text-white text-sm bg-gray-200 dark:bg-white/10 px-2 py-0.5 rounded-md">
                                        {item.activity_type}
                                    </span>
                                </div>
                            </div>
                        ...
                        <div className="flex justify-between items-center pt-2 border-t border-gray-200 dark:border-white/5 relative z-10">
                             <div className="flex items-center gap-1.5">
                                 ...
                                 <span className="text-[10px] text-gray-500 dark:text-gray-400">{item.sales_name}</span>
                             </div>
                        </div>
                    </div>
                    ))}
